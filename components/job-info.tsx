  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.valueContainer}>
          <Text style={styles.value}>
            {cropTextForDevice(job.airport?.name, 30)}
          </Text>
        </View>
      </View>
      <View style={styles.row}>
        ...
        <View style={styles.valueContainer}>
          <Text style={styles.value}>
            {cropTextForDevice(job.fbo?.name, 30)}
          </Text>
        </View>
      </View>
      <View style={styles.row}>
        ...
        <View style={styles.valueContainer}>
          <Text style={styles.value}>
            {job.on_site ? "On site" : job.arrival_formatted_date}
          </Text>
        </View>
      </View>
      <View style={styles.row}>
        ...
        <View style={styles.valueContainer}>
          <Text style={styles.value}>
            {job.departure_formatted_date || "Not specified"}
          </Text>
        </View>
      </View>
      ...
      {showMore && (
        <>
          <View style={styles.row}>
            ...
            <View style={styles.valueContainer}>
              <Text style={styles.value}>
                {job.complete_before_formatted_date || "Not specified"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.valueContainer}>
              <Text style={styles.value}>
                {job.customer_purchase_order || "Not specified"}
              </Text>
            </View>
          </View>
          <View style={styles.row}>
            ...
            >
              {job.requested_by ? (
                <Text style={styles.valueLight}>{job.requested_by}</Text>
              ) : (
                <>
                  <UserAvatar
                    avatar={job.created_by.profile?.avatar}
                    initials={`${job.created_by.first_name[0] ?? ""}${
                      job.created_by.last_name[0] ?? ""
                    }`}
                    size={30}
                  />
          ...
          {job.follower_emails?.length > 0 &&
            (currentUser.isAdmin ||
              ...
              <View style={styles.row}>
                ...
                <View style={styles.valueContainer}>
                  {job.follower_emails?.map(
                    (follower: { id: number; email: string }) => (
